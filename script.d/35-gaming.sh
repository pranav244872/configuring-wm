...
performance_tweaks() {
  ...
  echo ***REMOVED*** | sudo -S tee /etc/modules-load.d/ntsync.conf >/dev/null <<< "ntsync"
  echo ***REMOVED*** | sudo -S modprobe ntsync 2>/dev/null || true
  ok "ntsync module enabled"

  echo ***REMOVED*** | sudo -S tee /etc/sysctl.d/99-gaming.conf >/dev/null <<< "kernel.split_lock_mitigate=0"
